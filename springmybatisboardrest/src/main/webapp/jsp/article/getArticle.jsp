...
<p class="text-end">
	<c:if test="${sessionScope.ss_mid==article.mid}">
		<input type="button" value="수정" onclick="location.href='/article/updateArticleForm/${article.aid}';" class="btn btn-warning">&nbsp;
		<input type="button" value="삭제" onclick="if (confirm('삭제하시겠습니까?')) article.deleteArticle('${article.aid}');" class="btn btn-danger">&nbsp;
	</c:if>
